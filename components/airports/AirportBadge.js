...
const AirportBadge = ({ name, icon, slug }) => {
  return (
    <Link href={`/flight-to/${slug}`}>
      <div
        className={`flex justify-center px-4 items-center border border-transparent hover:border-sky-300 py-3 rounded-lg  cursor-pointer hover:text-sky-700 group ${styles.animateFly}`}
      >
